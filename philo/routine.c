...
/*   Updated: 2023/09/02 16:28:28 by hbechri          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*routine(void *arg)
{
	...
	while (1)
	{
		...
		if (data->philos->nb_eat_max != -1
			&& data->eat_count >= data->philos->nb_eat_max)
		{
			pthread_mutex_lock(&data->philos->mutex->end);
			data->philos->end_eating = 1;
...
void	threads_init(t_philos *philos)
{
	...
	{
		pthread_create(&philos->data[i].thread, NULL,
